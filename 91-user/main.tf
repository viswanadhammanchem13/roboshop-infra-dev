...
resource "terraform_data" "user"{
  triggers_replace = [ ##This trigger will cause the data source to be recreated whenever the user instance is replaced.
    ...
    provisioner "remote-exec" {
        ...
        "sudo sh /tmp/bootstrap.sh user ${var.environment}"
        ]
...
resource "terraform_data" "user_delete_instance"{
  triggers_replace = [
    aws_instance.user.id
  ]
  ##Makesure aws configuration is done in the system where you are running terraform apply, otherwise this command will fail. You can use AWS CLI or AWS Console to check the status of the instance and AMI creation.
...
resource "aws_launch_template" "user" {
  name = "${var.project}-${var.environment}-user"
  update_default_version = true ##Each time you run terraform apply, it will create a new version of the launch template with the updated AMI ID and it makes as default version, so that when you launch an instance using this launch template, it will use the latest version with the updated AMI ID. This ensures that any new instances launched using this launch template will have the latest AMI with the necessary configurations for the user service.
 ...
 ## Instance tags are important for cost allocation and resource management. By tagging the instances associated with the user, you can easily identify and track the costs associated with those instances in your AWS billing reports. Additionally, if you need to perform any maintenance or troubleshooting on the instances, having them tagged with relevant information (such as project name, environment, etc.) can help you quickly identify which instances are associated with the user and take appropriate actions. 
...
##Volume tags are important for cost allocation and resource management. By tagging the volumes associated with the user instances, you can easily identify and track the costs associated with those volumes in your AWS billing reports. Additionally, if you need to perform any maintenance or troubleshooting on the volumes, having them tagged with relevant information (such as project name, environment, etc.) can help you quickly identify which volumes are associated with the user instances and take appropriate actions.
    tag_specifications {
