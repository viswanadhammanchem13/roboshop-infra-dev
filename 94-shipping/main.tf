resource "aws_lb_target_group" "shipping" {
  name     = "${var.project}-${var.environment}-shipping" #roboshop-dev-shipping
  port     = 8080
  protocol = "HTTP"
  vpc_id   = local.vpc_id
  health_check {
...
# resource "terraform_data" "shipping_start_instance"{
#   triggers_replace = [
#     aws_instance.shipping.id
#   ]
...
#     command = "aws ec2 start-instances --instance-ids ${aws_instance.shipping.id}"
#   }

#   depends_on = [aws_ami_from_instance.shipping]
# }

resource "terraform_data" "shipping_delete_instance"{
  triggers_replace = [
    aws_instance.shipping.id
  ]
  ##Makesure aws configuration is done in the system where you are running terraform apply, otherwise this command will fail. You can use AWS CLI or AWS Console to check the status of the instance and AMI creation.
...
resource "aws_launch_template" "shipping" {
  name = "${var.project}-${var.environment}-shipping"
  update_default_version = true ##Each time you run terraform apply, it will create a new version of the launch template with the updated AMI ID and it makes as default version, so that when you launch an instance using this launch template, it will use the latest version with the updated AMI ID. This ensures that any new instances launched using this launch template will have the latest AMI with the necessary configurations for the shipping service.
 ...
 ## Instance tags are important for cost allocation and resource management. By tagging the instances associated with the shipping service, you can easily identify and track the costs associated with those instances in your AWS billing reports. Additionally, if you need to perform any maintenance or troubleshooting on the instances, having them tagged with relevant information (such as project name, environment, etc.) can help you quickly identify which instances are associated with the shipping service and take appropriate actions. 
...
##Volume tags are important for cost allocation and resource management. By tagging the volumes associated with the shipping instances, you can easily identify and track the costs associated with those volumes in your AWS billing reports. Additionally, if you need to perform any maintenance or troubleshooting on the volumes, having them tagged with relevant information (such as project name, environment, etc.) can help you quickly identify which volumes are associated with the shipping instances and take appropriate actions.
    tag_specifications {
...
resource "aws_lb_listener_rule" "shipping" {
  ...
  condition {
    host_header {
      values = ["shipping.backend-${var.environment}.${var.zone_name}"]
    }
  }
}
